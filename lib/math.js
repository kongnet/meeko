...
const genRange = (s, e, step = 1) =>
  Array.from({ length: e - s + 1 }, (r, i) => i + s + (step - 1) * i)
const max = a => {
  /**
...
const quantile = (a, pos = 2, type = 'inc') => {
  ...
  switch (+pos) {
    ...
    case 1:
      ...
      return (
        meSort[sit] +
        (up - meSort[sit]) * ratio[type === 'inc' ? lowSit % 4 : 1]
      )
    case 2:
    ...
    case 3:
      ...
      return (
        meSort[sit] + (up - meSort[sit]) * ratio[type === 'inc' ? upSit % 4 : 3]
      )
    case 4:
...
const quantileAll = (a, type = 'inc') => {
  ...
  const Q1 =
    meSort[sit] + (up - meSort[sit]) * ratio[type === 'inc' ? lowSit % 4 : 1]
  sit = ((upSit / 4) | 0) - 1
  up = meSort[sit + 1]
  if (up === undefined) {
    up = meSort[sit]
...
const findMaxMin = a => {
  ...
  if (typeof a[0] === 'bigint') {
    [max, min, maxIdx, minIdx, sum, count] = [
      MIN64_BIGINT,
      MAX64_BIGINT,
      -1,
      -1,
...
module.exports = Object.assign.call(null, mathAlgebra, mathRand, mat, {
  ...
  exponentFitting (x = [], y = []) {
    ...
    return {
      ...
      latex: `y=${a.toFixed(4)}\times e^{ (${b.toFixed(
        4
      )}\times x)} R^{2}=${r.toFixed(4)}`
    }
  },
  lnFitting (x = [], y = []) {
    // 对数拟合 y=a*ln(x)+b
