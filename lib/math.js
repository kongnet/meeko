...
const quartile = (a, pos = 2, type = 'inc') => {
  /**
   * @memberof Math_prototype#
   * @param {Array} a - 数组
   * @param {integer} pos - 1四分之一 2四分之二 3四分之三
  ...
  const sit =
    (len + (type === 'inc' ? -1 : 1)) * [null, 0.25, 0.5, 0.75][pos] +
    1 * (type === 'inc' ? 1 : 0)

  if ((sit | 0) === sit) {
    return meSort[sit - 1]
  } else {
    return (meSort[(sit | 0) - 1] + meSort[sit | 0]) / 2
  }
}
const median = a => {
  /**
...
const mode = a => {
  ...
  let arr = []
  let middle = (a.length / 2) | 0
  let o = count(a)
  for (const i in o) {
    if (o[i] > middle) arr.push(+i)
  }
  return arr
}
...
// 数组原型扩展
module.exports = Object.assign.call(null, mathAlgebra, mathRand, {
  fac,
  arrangement,
  combination,
  hCombin,
