...
const median = a => {
  ...
  return len % 2 === 0 ? (meSort[len / 2] + meSort[len / 2 - 1]) / 2 : meSort[~~(len / 2)]
}
const count = (a = []) => {
  /**
   * @memberof Math_prototype#
...
// 数组原型扩展
module.exports = Object.assign.call(
  null,
  mathAlgebra,
  mathRand,
  {
    approximatelyEqual (v1, v2, epsilon = 0.001) { return Math.abs(v1 - v2) < epsilon },
    count,
    sum,
    max,
    min,
    ...
    polyFitting (x = [], y = [], n = 2) { // 多项式拟合
      ...
      return {
        r,
        f: `y=${formula.reverse().join('+').replace('^1+', '')} R^2=${r.toFixed(4)}`
      }
    },
    pearson (x = [], y = []) {
      /**
