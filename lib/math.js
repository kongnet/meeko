...
* // '6.465714'
*/
const varianceCorrect = a => { // sqrt(varianceCorrect) 标准偏差 2.542777 更正开根号
  let mean = a.mean()
  let len = a.length - 1
  return a.reduce(function (x, y) {
    return x + (y - mean) * (y - mean)
...
* // 1.4142135623730951
*/
const stddev = a => Math.sqrt(variance(a))// stddev 总体标准偏差(population)/标准差/均方差 2.354154 方差开
...
* // '2.542777'
*/
const stddevCorrect = a => Math.sqrt(varianceCorrect(a))// 标准偏差(sample)
