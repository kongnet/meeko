...
const checkIdCard = {
  ...
  checkAddressCode: function (addressCode) {
    if (!addrCodeArr) loadFakeFile()
    if (addrCodeArr.includes(+addressCode)) {
      return true
    } else {
      return false
    }
  ...
  checkBirthDayCode: function (birDayCode) {
    ...
    let yyyy = parseInt(birDayCode.slice(0, 4), 10)
    let mm = parseInt(birDayCode.slice(4, 6), 10)
    let dd = parseInt(birDayCode.slice(6), 10)
  ...
  check15IdCardNo: function (idCardNo) {
    ...
    if (!check) return false
    //校验地址码
    let addressCode = idCardNo.slice(0, 6)
    check = checkIdCard.checkAddressCode(addressCode)
    if (!check) return false
    let birDayCode = '19' + idCardNo.slice(6, 12)
    //校验日期码
  ...
  check18IdCardNo: function (idCardNo) {
    ...
    if (!check) return false
    //校验日期码
    let birDayCode = idCardNo.slice(6, 14)
  ...
  getInfo: function (idCardNo) {
    ...
    if (idCardNo.length == 15) {
      let aday = '19' + idCardNo.slice(6, 12)
    ...
    let address = idCardNo.slice(0, 6)
    idCardInfo.place = [
  ...
  getId18: function (idCardNo) {
    if (idCardNo.length == 15) {
      let id17 = idCardNo.slice(0, 6) + '19' + idCardNo.slice(6)
      let parityBit = checkIdCard.getParityBit(id17)
