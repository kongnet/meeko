...
(function (Math) {
  ...
  miniColor.prototype = {
    ...
    toHsvString: function () {
      const hsv = rgbToHsv(this._r, this._g, this._b)
      const h = mathRound(hsv.h * 360); const s = mathRound(hsv.s * 100); const v = mathRound(hsv.v * 100)
      return this._a == 1
        ? 'hsv(' + h + ', ' + s + '%, ' + v + '%)'
    ...
    toHslString: function () {
      const hsl = rgbToHsl(this._r, this._g, this._b)
      const h = mathRound(hsl.h * 360); const s = mathRound(hsl.s * 100); const l = mathRound(hsl.l * 100)
      return this._a == 1
        ? 'hsl(' + h + ', ' + s + '%, ' + l + '%)'
  ...
  function rgbToHsl (r, g, b) {
    ...
    const max = mathMax(r, g, b); const min = mathMin(r, g, b)
  ...
  function triad (color) {
    ...
    return [
      miniColor(color),
      miniColor({ h: (h + 120) % 360, s: hsl.s, l: hsl.l }),
      miniColor({ h: (h + 240) % 360, s: hsl.s, l: hsl.l })
    ]
  }
  ...
  function tetrad (color) {
    ...
    return [
      miniColor(color),
      miniColor({ h: (h + 90) % 360, s: hsl.s, l: hsl.l }),
      miniColor({ h: (h + 180) % 360, s: hsl.s, l: hsl.l }),
      miniColor({ h: (h + 270) % 360, s: hsl.s, l: hsl.l })
    ]
  }
  ...
  function splitcomplement (color) {
    ...
    return [
      miniColor(color),
      miniColor({ h: (h + 72) % 360, s: hsl.s, l: hsl.l }),
      miniColor({ h: (h + 216) % 360, s: hsl.s, l: hsl.l })
    ]
  }
  ...
  function monochromatic (color, results) {
    ...
    const h = hsv.h; const s = hsv.s; let v = hsv.v
    const ret = []
  ...
  miniColor.mostReadable = function (baseColor, colorList, args) {
    ...
    if (miniColor.isReadable(baseColor, bestColor, { level: level, size: size }) || !includeFallbackColors) {
      return bestColor
    } else {
  ...
  function stringInputToObject (color) {
    color = color.replace(trimLeft, '').replace(trimRight, '')
      .toLowerCase()
    let named = false
    if (names[color]) {
      color = names[color]
  ...
  if (typeof module !== 'undefined' && module.exports) {
    ...
    define(function () { return miniColor })
  } else {
