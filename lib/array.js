  ...
  groupBy(groupCol, aggregateCol = [], aggregateOpt = [], aggregateColAlias = []) {
        ...
        aggregateCol.forEach(item => {
          ...
          switch (opt) {
            ...
            case 'sum':
              ...
              break
            case 'avg':
              keyObj[`avgCount(${item})`]++
              keyObj[aggregateColAlias[idx] ?? `avg(${item})`] =
                (keyObj[aggregateColAlias[idx] ?? `avg(${item})`] * (keyObj[`avgCount(${item})`] - 1) + aItem[item]) / keyObj[`avgCount(${item})`]
              break
            case 'max':
              keyObj[aggregateColAlias[idx] ?? `max(${item})`] =
                keyObj[aggregateColAlias[idx] ?? `max(${item})`] < aItem[item]
                  ? aItem[item]
                  : keyObj[aggregateColAlias[idx] ?? `max(${item})`]
              break
            case 'min':
              keyObj[aggregateColAlias[idx] ?? `min(${item})`] =
    ...
    for (const i in groupObj) {
      ...
      for (let d = 0; d < groupColKey.length; d++) {
        if (groupCol[d]) o[groupCol[d]] = groupColKey[d]
      }
      for (const k in groupObj[i]) {
        if (!k.includes('avgCount')) o[k] = groupObj[i][k]
