        ...
        aggregateCol.forEach(item => {
          ...
          switch (opt) {
            ...
            case 'sum':
              ...
              break
            case 'avg':
              keyObj[`avg(${item})`] = (keyObj[`avg(${item})`] + aItem[item]) / 2
              break
            case 'max':
              keyObj[`max(${item})`] = keyObj[`max(${item})`] < aItem[item] ? aItem[item] : keyObj[`max(${item})`]
              break
            case 'min':
              keyObj[`min(${item})`] = keyObj[`min(${item})`] > aItem[item] ? aItem[item] : keyObj[`min(${item})`]
              break
            default:
          }
          idx++
