...
class Snowflake {
  constructor (_workerId, _dataCenterId, _sequence) {
    ...
    this.maxWorkerId = -1n ^ (-1n << this.workerIdBits) // 值为：31
    ...
    if (this.workerId > this.maxWorkerId || this.workerId < 0) {
      throw new Error('0 < _workerId < maxWorkerId-[' + this.maxWorkerId + ']')
    }
  ...
  nextId () {
    ...
    if (timestamp < this.lastTimestamp) {
      console.error(new Error('时钟倒退了，无法产生id ' + (this.lastTimestamp - timestamp)))
