...
const tools = {
   ...
   */
    ...
    return keys.every(k => tools.equals(a[k], b[k]))
  },
  getType (o) {
    /**
...
tools.wait = function (t) {
   ...
   * @return {Function}
   */
  return new Promise((resolve) => { // 老的方法使用 return function 在最外层递归调用时候有问题
...
tools.waitNotEmpty = async function (o, prop, fn) {
  ...
  fn = fn || function () { /* do nothing */ }
  if (!o[prop]) {
    fn(o, prop)
    await tools.wait(100)
    await tools.waitNotEmpty(o, prop, fn)
  }
}
...
tools.timeAgo = function (t1, t2) {
   ...
   * @function timeAgo
   * @param {Date|int|String} t1 - 时间随便取
   * @param {Date|int|String} t2 - 时间随便取
   * @return {String}
   * @example
