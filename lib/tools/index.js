    ...
    switch ((valB.type || 'string').toLow()) {
      ...
      case 'string':
        if (typeof valA !== 'string') {
          return {
            code: 401,
            msg: (valB.name || i) + ' Type error, must be String, not a Null or Number' //' 类型错误,应为字符串，不能为null NaN等'
          }
        }
        ...
        if (valA && (valB.regex || valB.reg)) {
          if (!new RegExp(valB.regex || valB.reg).test(valA)) {
            return {
              code: 401,
      ...
      case 'object': // 支持对象
        if (typeof valA !== 'object') {
          return {
            code: 401,
            msg: (valB.name || i) + ' Type error, must be object' //' 类型错误,应为object型'
          }
        }
        addToC && (c[i] = valA)
        break
  ...
  for (let i = 0; i < len; i++) {
    ...
    for (let k = 0; k < keysLen; k++) {
      ...
      if (opt.color) {
        ...
        const diffLen = /\x1b\[[0-9;]*m/g.test(v) ? 13 : 0
 ...
 */
function roulette (optionArr = [{ w: 0 }]) {
