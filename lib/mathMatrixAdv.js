...
function subMatrix (a, startRow, endRow, startColumn, endColumn) {
  let newMatrix = Mat.zero(endRow - startRow + 1, endColumn - startColumn + 1)
...
class Matrix {
  ...
  shape () {
    let row = this.optMatrix.length
    let col = this.optMatrix[0].length
    return { row, col }
  }
  //行列式
  det () {
  ...
  colStat (a, colNum = 0) {
    a = a || this.optMatrix
    let r = a.map((x, idx) => a[idx][colNum])
    return {
      mean: $M.mean(r),
      std: $M.stddevCorrect(r),
      median: $M.median(r),
  ...
  trace (a) {
    a = a || this.optMatrix
    let min = Math.min(a.length, a[0].length)
    let trace = 0
    for (let i = 0; i < min; i++) {
      trace += a[i][i]
    }
  ...
  print (a) {
    a = a || this.optMatrix
    let colMaxWidth = $M.genRange(0, a[0].length, 0)
    let data = a.map((x, idx) => {
      let rowObj = {}
      for (let i = 0; i < x.length; i++) {
        let col1 = this.colName[idx] || `[${idx + 1},]`
        rowObj['-'] = col1
        colMaxWidth[0] =
...
class QrDc {
  ...
  get orthogonalMatrix () {
    let qr = this.QR
    let rows = qr.length
    let columns = qr[0].length
    let X = Mat.zero(rows, columns)
    let i, j, k, s
