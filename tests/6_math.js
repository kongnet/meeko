...
describe('Math扩展函数的单元测试', () => {
  ...
  it('hMean 调和平均数 2.742815', () =>
    assert.strictEqual($.math.hMean(a).toFixed(6), '2.742815')
  )
  it('gMean 几何平均数 3.515999', () =>
    assert.strictEqual($.math.gMean(a).toFixed(6), '3.515999')
  )

  it('mean (算术)平均数 4.271429', () =>
    assert.strictEqual($.math.mean(a).toFixed(6), '4.271429')
  )
  it('qMean 平方平均数 4.877206', () =>
    assert.strictEqual($.math.qMean(a).toFixed(6), '4.877206')
  )
  it('min 最小', () =>
    assert.strictEqual($.math.min(a), 1)
  )
  it('max 最大', () =>
    assert.strictEqual($.math.max(a), 8.1)
  )
  it('range 极值', () =>
    assert.strictEqual($.math.range(a), 7.1)
  )
  it('median 中位数 4', () => {
    assert.strictEqual($.math.median(a), 4)
  ...
  it('mean Deviation 平均偏差 2.024490', () =>
    assert.strictEqual($.math.meanDev(a).toFixed(6), '2.024490')
  )
  it('medianDev 中位数偏差 1.985714', () =>
    assert.strictEqual($.math.medianDev(a).toFixed(6), '1.985714')
  )
  it('Standard error 标准误差 0.961079', () =>
    assert.strictEqual($.math.stdErr(a).toFixed(6), '0.961079')
  )
  it('Coefficient of Variation 变异系数/离散系数 0.551140', () =>
    assert.strictEqual($.math.coeVariation(a).toFixed(6), '0.551140')
  )
  it('confidence intervals 置信区间', () =>
    assert.strictEqual($.math.confidenceIntervals(a)[0] + $.math.confidenceIntervals(a)[1], 0.1659026888164572 + 8.376954454040686)
  )
  /*
  $.math.linearFitting()
...
describe('Math偏度和3个峰度函数单元测试', () => {
  ...
  it('skew 偏度 pandas模块 0.7826325504212567', () =>
    assert.strictEqual($.math.skew(a).toFixed(6), '0.782633')
  )
  it('kurt1 峰度/峰态系数 pandas模块 Estimators of population kurtosis人口峰度 -0.2631655441038472', () =>
    assert.strictEqual($.math.kurt1(a), -0.2631655441038472)
  )
  it('kurt2 >3 =3 <3 峰度/峰态系数 1.7105241302560437', () =>
    assert.strictEqual($.math.kurt2(a), 1.7105241302560437)
  )
  it('kurt wiki上的算法 峰度/峰态系数 -0.9473710436927472', () =>
    assert.strictEqual($.math.kurt3(a), -0.9473710436927472)
  )
})
